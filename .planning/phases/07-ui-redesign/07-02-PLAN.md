...
- All text strings must use `t()` for i18n. Add new translation keys to both `front/src/locale/lang.fr.json` and `front/src/locale/lang.ar.json`: "per order" / "products" / "Low Stock" / "Now" / "Revenue by Category" / "Revenue by Vendor" / "Profit by Product"
...
1. **Import the necessary dependencies:**
   ...
   - `USER_LIST`, `USER_CREATE` from the admin routing: `../../../api/routing/routes/admin.backend.app` (which exports `USER_LIST = adminScopeUrl('user/list')` and `USER_CREATE = adminScopeUrl('user/create')`)
...
7. **Toggle user active status:**
   Port the activate/deactivate toggle from the POS users component. Use a switch or button that calls the user edit endpoint with `isActive: !currentValue`.
...
- Add translation keys: "Create User", "Edit User", "Display Name", "Username", "Password", "Role", "Active", "Inactive", "Vendor", "Manager", "Admin" -- most of these already exist in lang files from Phase 1 POS-side implementation, but verify and add any missing ones to both `lang.fr.json` and `lang.ar.json`
