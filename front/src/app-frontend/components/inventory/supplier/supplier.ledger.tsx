      ...
      >
        <form onSubmit={handleSubmit(createPayment)} className="mb-5">
          ...
          <div className="grid grid-cols-5 gap-4 mb-3">
            ...
            <div className="col-span-1">
              <label htmlFor="purchase">{t("Purchase no.")}</label>
              <Controller
                control={control}
                name="purchase"
                render={(props) => (
            ...
            <div>
              ...
              <Button variant="primary" type="submit" className="w-full"
                      disabled={creating}>{creating ? t('Adding...') : t('Add Payment')}</Button>
            </div>
          </div>
        </form>
          ...
          {list.map((item: any, index: number) => (
            <tr key={index} className="hover:bg-gray-100">
              ...
              <td>
                {item.amount && (
                  <>
                    {t("Payment")} {withCurrency(item.amount)}
                  </>
                )}
                {item.purchaseNumber && (
                  <>
                    {item?.paymentType?.name} {t("Sale")}: {withCurrency(item.total)}
                  </>
                )}
              </td>
              <td>
                {item.description && (
                  <>{t("Receiving")}: {item.description}</>
                )}
                {item.purchaseNumber && t('Sale')}
              </td>
              <td>
                {item.purchaseNumber && (
