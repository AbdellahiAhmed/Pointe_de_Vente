...
export const CreatePurchaseOrder: FunctionComponent<CreatePurchaseOrderProps> = ({
  ...
  return (
    <>
      <Modal
        open={modal}
        size="full" title={operation === 'create' ? t("Create purchase order") : t('Update purchase order')}
      ...
      >
        <form onSubmit={handleSubmit(createPurchaseOrder)} className="mb-5">
          ...
          <div className="grid lg:grid-cols-4 gap-4 mb-3 md:grid-cols-3 sm:grid-cols-1">
            ...
            <div>
              <label htmlFor="poNumber">{t("PO Number")}</label>
          ...
          <div>
            <Button variant="primary" type="submit" disabled={creating}>
              {creating ? t('Saving...') : (operation === 'create' ? t('Add purchase order') : t('Update purchase order'))}
            </Button>
          </div>
        </form>
      </Modal>
          ...
          <div className="grid grid-cols-5 gap-3 mb-1 px-5 hover:bg-gray-200">
            ...
            <div className="inline-flex items-center">
              <ConfirmAlert
                ...
                title={`${t("Remove")} ${item.item.name} > ${variant.attributeName}?`}
                confirmText={t("Remove")}
              >
                <button className="btn btn-danger" type="button">
                  <FontAwesomeIcon icon={faRemove}/>
