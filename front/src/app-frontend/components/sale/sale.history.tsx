...
export const SaleHistory: FC<Props> = ({}) => {
  ...
  const unsuspendOrder = async (order: Order) => {
    if (!window.confirm(t("Unsuspend order?"))) return false;
  ...
  const deleteOrder = async (order: Order) => {
    if (!window.confirm(t("Delete order?"))) return false;
    setDeleting(true);
    try {
      await fetchJson(ORDER_GET.replace(":id", order.id), {
        method: "DELETE",
  ...
  const restoreOrder = async (order: Order) => {
    if (!window.confirm(t("Restore order?"))) return false;
    setRestoring(true);
    try {
    ...
    <>
      <Tooltip title={t("Sale lookup")}>
        ...
        >
          ...
          <Shortcut actionId="open_history" handler={() => setModal(true)} />
        </Button>
      </Tooltip>
        ...
        <form onSubmit={handleSubmit(searchSale)}>
          <div className="grid grid-cols-6 gap-4 mb-5">
            <div className="col-span-3">
              <Input
                type="search"
                placeholder={t("Search in Order#, Status, Customer")}
          ...
          <>
            ...
            {areChartsOpen && (
              <div className="mb-5 grid grid-cols-3 gap-4">
                <div>
                  <h4 className="text-lg">{t("Payment types")}</h4>
                ...
                <div>
                  <h4 className="text-lg">{t("Sales")}</h4>
                  <div className="h-[300px]">
                    <Bar
                      data={[
                ...
                <div>
                  <h4 className="text-lg">{t("Customers")}</h4>
                  <div className="h-[300px]">
                    <Bar
