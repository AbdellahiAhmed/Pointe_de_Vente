        ...
        <>
          ...
          {info.getValue().slice(7).length > 0 && (
            <Button
              ...
              type="button"
            >+{info.getValue().slice(7).length} {t("more")}
            </Button>
          )}
        </>
      )
  ...
  return (
    <>
      ...
      <Modal open={terminalProducts.length > 0} onClose={() => {
        setTerminalProducts([]);
      }} title={t("Terminal products")} size="full">
        <div className="w-full mb-3">
          <Input type="search" placeholder={t("Filter products")} onChange={(e) => setFilter(e.target.value)}
                 value={filter}/>
        </div>
